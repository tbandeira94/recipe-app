...
import { SearchPage, type SearchState } from './pages/SearchPage'
...
const emptySearchState = (): SearchState => ({ mode: 'recipes', query: '', ingredientValue: '', ingredients: [], matchMode: 'all', recipeResultLimit: 40, ingredientResultLimit: 40 })
...
export default function App() {
  ...
  const openRecipe = (id: string, from: Tab = activeTab) => {
    tabScrollPositions.current[from] = window.scrollY
    pendingScrollPosition.current = 0
    setView({ kind: 'details', id, from })
  }
  const showTab = (tab: Tab) => {
    if (view.kind === 'tab') tabScrollPositions.current[view.tab] = window.scrollY
    pendingScrollPosition.current = tabScrollPositions.current[tab] ?? 0
    setView({ kind: 'tab', tab })
  }
  const showNewViewAtTop = (nextView: View) => {
    if (view.kind === 'tab') tabScrollPositions.current[view.tab] = window.scrollY
    pendingScrollPosition.current = 0
    setView(nextView)
  }
  ...
  const content = (() => {
    ...
    if (view.kind === 'details' && selected) return <RecipeDetailsPage recipe={selected} onBack={() => showTab(view.from)} onEdit={() => showNewViewAtTop({ kind: 'form', id: selected.id, from: view.from })} onToggleFavorite={() => void toggleFavorite(selected)} onDelete={async () => { if (confirm(`Delete “${selected.name}”?`)) { await deleteRecipe(selected.id); await refresh(); showTab('recipes') } }} />
    if (view.kind === 'import') return <RecipeImportPage value={importInput} error={importError} onChange={(input) => { setImportInput(input); setImportError('') }} onCancel={() => showTab(view.from)} onReview={() => {
      try {
        const result = parseRecipeText(importInput.text, importInput)
        setImportError('')
        showNewViewAtTop({ kind: 'form', from: view.from, initialDraft: result.draft, importWarnings: result.warnings })
      } catch (error) { setImportError(error instanceof Error ? error.message : 'Couldn’t read that recipe text.') }
    }} />
    if (view.kind === 'form' && (!view.id || selected)) return <RecipeFormPage recipe={selected} initialDraft={view.initialDraft} importWarnings={view.importWarnings} dishTypeSuggestions={dishTypeSuggestions} tagSuggestions={tagSuggestions} onCancel={() => view.id ? showNewViewAtTop({ kind: 'details', id: view.id, from: view.from }) : view.initialDraft ? showNewViewAtTop({ kind: 'import', from: view.from }) : showTab(view.from)} onSave={async (draft: RecipeDraft, photoUpdate: PhotoUpdate) => { const saved = await saveRecipe(draft, photoUpdate, selected); if (photoUpdate.kind === 'replace') void requestPersistentStorage(); await refresh(); if (view.initialDraft) setImportInput(emptyImportInput()); showNewViewAtTop({ kind: 'details', id: saved.id, from: view.from }) }} />
    if (view.kind !== 'tab') return <div className="page"><div className="status-card">Opening recipe…</div></div>
    if (view.tab === 'search') return <SearchPage recipes={recipes} searchState={searchState} onSearchStateChange={setSearchState} onOpen={(id) => openRecipe(id, 'search')} onToggleFavorite={(recipe) => void toggleFavorite(recipe)} />
  ...
  return <><Layout activeTab={activeTab} onTabChange={(tab) => { setAddChoiceOpen(false); showTab(tab) }} updateAvailable={Boolean(waitingWorker)} onUpdate={() => waitingWorker?.postMessage({ type: 'SKIP_WAITING' })}>{content}</Layout>
    {addChoiceOpen && <AddRecipeDialog onClose={() => setAddChoiceOpen(false)} onManual={() => { setAddChoiceOpen(false); showNewViewAtTop({ kind: 'form', from: 'recipes' }) }} onImport={() => { setAddChoiceOpen(false); setImportError(''); showNewViewAtTop({ kind: 'import', from: 'recipes' }) }} />}
  </>
}
